...
class CodeWindow():
    def __init__(self, info_dict: dict, type: str):
        ...
        # Remove angle brackets in inline_labels
        for i, label in enumerate(self.inline_labels):
            if label.startswith("<") and label.endswith(">"):
                self.inline_labels[i] = label[1:-1]

        # Remove angle brackets in inter_labels
        for i, label in enumerate(self.inter_labels):
            if label.startswith("<") and label.endswith(">"):
                self.inter_labels[i] = label[1:-1]

        assert set(self.inline_labels).issubset(set(["keep", "replace", "delete"])), f"Invalid inline labels: {set(self.inline_labels)}"
        assert set(self.inter_labels).issubset(set(["null", "insert", "block-split"])), f"Invalid inter labels: {set(self.inter_labels)}"
    ...
    def formalize_as_generator_target_window(self, beautify: bool = False, label_num: int = 6):
        if self.window_type == "sliding_window": # object is a sliding window
            if label_num == 3:
                from .utils import label_conversion
                labels = label_conversion(self.inline_labels, self.inter_labels)
                assert len(labels) == len(self.code_window)
                 
                if beautify:
                    target_window = f"<code_window>"
                else:
                    target_window = f"<code_window>"
                for label, code_line in zip(labels, self.code_window):
                    if beautify:
                        code_line = code_line.replace("\n", "")
                        target_window += f"\n\t<{label}>{code_line}"
                    else:
