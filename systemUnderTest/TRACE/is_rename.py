...
def lcs(list1, list2):
    ...
    # Create a (m+1) x (n+1) 2D array initialized to 0
    ...
    # Backtrack to find the LCS and merge elements
    merged_list = []
    i, j = m, n
    while i > 0 and j > 0:
        if list1[i - 1]['text'] == list2[j - 1]['text'] and list1[i - 1]['type'] == list2[j - 1]['type']:
...
def merge_matched_position(common_positions):
    ...
    def is_consecutive(numbers):
        if len(numbers) < 2:
            return True  # 0 or 1 element is considered consecutive
